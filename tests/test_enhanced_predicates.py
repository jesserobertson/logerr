...
class TestOptionPredicateEnhancements:
    ...
    def test_from_predicate_with_custom_error_message(self):
        ...
        option = logerr.option.from_predicate(5, lambda x: x > 30, error_message="Number too small")
...
class TestResultPredicateEnhancements:
    ...
    def test_from_predicate_with_exception(self):
        ...
        result = logerr.result.from_predicate("not a number", lambda s: int(s) > 0, "conversion error")
        assert result.is_err()
    ...
    def test_predicate_validator_factory(self):
        ...
        # Create a reusable validator
        validate_positive = logerr.result.predicate_validator(lambda x: x > 0, "must be positive")
    ...
    def test_predicate_validator_chaining(self):
        ...
        validate_even = logerr.result.predicate_validator(lambda x: x % 2 == 0, "must be even")
        validate_positive = logerr.result.predicate_validator(lambda x: x > 0, "must be positive")
    ...
    def test_mixed_error_types(self):
        ...
        # String error
        validate_string = logerr.result.predicate_validator(lambda x: x > 0, "negative number")
