...
import logerr
from logerr import Some, Nothing, Ok, Err, Option, Result
...
def option_values(draw):
    ...
    return draw(st.one_of(
        st.just(Some(value_type)),
        st.just(Nothing.empty())
    ))
...
class TestRealisticScenarios:
    ...
    @given(st.dictionaries(st.text(min_size=1, max_size=10), st.integers(), min_size=0, max_size=20))
